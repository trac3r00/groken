...
async def test_poller_enrolls_leases_executes_and_completes(tmp_path: Path) -> None:
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        if request.url.path == "/v1/worker/complete":
            completed_payloads.append(cast("dict[str, object]", json.loads(request.content)))
            return httpx.Response(202, json={"accepted": True})
        raise AssertionError(f"unexpected path: {request.url.path}")

    client = httpx.AsyncClient(transport=httpx.MockTransport(handler), base_url="https://controller.test")
    runner = FakeRunner()
...
@pytest.mark.anyio
async def test_poller_refuses_changed_controller_before_sending_token(tmp_path: Path) -> None:
    requests: list[str] = []
    ...
    first_client = httpx.AsyncClient(
        transport=httpx.MockTransport(first_handler), base_url="https://controller-a.test"
    )
    first = RemotePoller(
        PollerSettings(
    ...
    second_client = httpx.AsyncClient(
        transport=httpx.MockTransport(second_handler), base_url="https://controller-b.test"
    )
